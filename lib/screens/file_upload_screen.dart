...
class _FileUploadScreenState extends State<FileUploadScreen> {
  ...
  Widget _uploadMediaButton(BuildContext context) {
    return FloatingActionButton(
      onPressed: () async {
        File? selectedImage = await AppUtils.getImageFromGallery(context);
        if (selectedImage != null) {
          bool success = await AppUtils.uploadFileForUser(selectedImage);
          if (success) {
            getUploadedFiles();
          }
        }
  ...
  void getUploadedFiles() async {
    List<Reference>? result = await AppUtils.getUsersUploadedFiles();
    if (result != null) {
      setState(
        () {
